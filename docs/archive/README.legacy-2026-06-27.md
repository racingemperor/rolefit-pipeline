...
Concrete recommended jobs, internships, or application targets must include real public URLs. The pipeline can use official entrypoints in `data/company_signals/official_application_entrypoints.zh-CN.json` for exploration and public JD/JD text for prepare-first recommendations. Exact fit scores, final application priority, company-specific weights, apply-now decisions, and tailored resume advice require stronger JD/user evidence gathered by the runtime source adapter. If a public page does not state opening status, city, onsite days, deadline, headcount, or internship duration, the user-facing package should add `ask_hr_about` instead of blocking the recommendation. See `.agents/skills/career-pipeline/references/application-url-output-policy.md` and `.agents/skills/career-pipeline/references/real-user-deployment-and-use-flow.md`.
  ...
  skills/
    career-pipeline/
      ...
      scripts/                  # local contract validation and simulation helpers
        ...
        build_subagent_prompt_bundle.py
...
- `scripts/` 当前提供本地合同验证、无网络模拟运行、plan-only subagent 调用队列、subagent prompt bundle、公开来源研究计划、安全执行外壳、角色输出回填校验和同一 run 的用户补充信息继续流程，用来检查 prompt、二次注入、subagent invocation、manifest、source policy、blocked/final gate 是否符合协议；这些脚本不代表真实 subagent 已执行。简历解析、JD 标准化、匹配证据矩阵和文档渲染仍是后续确定性脚本扩展。
