...
- `scripts/` 是后续计划中的确定性任务目录，例如简历解析、JD 标准化、schema 校验、匹配证据矩阵生成、文档渲染；当前阶段先搭 prompt 和数据库，不实际部署脚本。
...
运行时还需要显式维护 `run_state`，用状态机保证不会跳过归一化、上下文包、二次注入、子智能体执行、合并、辩论、HR 监督、事实审查和用户确认。用户端交互遵循“一轮补充”原则：先告诉用户已知信息、现在能做什么、暂时不能做什么，再只询问用户本人才能提供的缺失事实。所有技能、外部展示、学校信号、投递策略和 HR 初筛权重都通过 `runtime_weights` 表达，证据不足时必须是 `not_available` 或 `needs_more_sources`。
