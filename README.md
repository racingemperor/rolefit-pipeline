...
- **已有简历打磨**：用户给出自己的简历之后，默认保留原格式进行打磨；如果仓库内置模板更具体、更有说服力，就把有证据支撑的内容补进原简历，而不是强行换模板。
- **个人展示资产建设**：用户授权后，可以帮助规划、生成或修改个人网站、GitHub/Gitee、项目 README、作品集、Demo 等能体现个人能力的公开链接。
- **多子智能体协作**：协调 17 个角色 prompts/subagents，并使用覆盖 85 家工科热门雇主的种子公司信号数据库。
...
- 用户已有简历的保格式打磨版。
- 完成学习或项目后的简历预览版。
- 经用户授权后生成或修改的个人展示资产方案，例如个人网站、GitHub/Gitee、项目 README、作品集或 Demo 链接。
...
这样可以让开场白先说明这个 Skill 的作用，并告诉用户接下来需要提供哪些信息。用户不需要提前组织完整材料，也不需要理解 subagent、JSON、runner 或 adapter。

开场白会说明 RoleFit Pipeline 可以做职业方向判断、岗位匹配、能力差距分析、项目/学习规划、简历修改、定制简历设计、个人网站设计，以及 GitHub/Gitee/作品集等个人展示资产优化。之后用户再按开场白一次性补充能提供的信息即可。

如果用户已经准备好了材料，也可以直接把 `$career-pipeline` 和个人情况写在同一条消息里。
